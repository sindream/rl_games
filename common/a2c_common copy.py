...
class A2CBase(BaseAlgorithm):
    ...
    def set_weights(self, weights):
        ...
        #     else:
        #         print(f"{name}: {param}")  # 텐서가 아닌 다른 데이터 타입 출력
    ...
    def play_steps_rnn(self):
        ...
        for n in range(self.horizon_length):
            ...
            print("rnn_states", self.rnn_states.shape)  
            exit()
            self.experience_buffer.update_data('obses', n, self.obs['obs'])
            # self.experience_buffer.update_data('obses', n, self.obs['obs'])
...
class ContinuousA2CPriv(ContinuousA2CBase):
    ...
    def train_epoch(self):
        ...
        for mini_ep in range(0, self.mini_epochs_num):
            ep_kls = []
            for i in range(len(self.dataset)):
                # print("self.dataset[i]", self.dataset[i].keys())
    ...
    def prepare_dataset(self, batch_dict):
        ...
        # print("batch_dict", batch_dict.keys()) # batch_dict dict_keys(['actions', 'neglogpacs', 'values', 'mus', 'sigmas', 'est_states', 'obses', 'dones', 'privileged_obses', 'estimate_states', 'rewards', 'returns', 'rnn_states', 'step_time'])
        obses = batch_dict['obses']
        returns = batch_dict['returns']
        dones = batch_dict['dones']
        values = batch_dict['values']
    ...
    def train(self):
        ...
        rep_count = 0
        self.obs = self.env_reset()
        # print("first reset self.obs", self.obs)
        self.curr_frames = self.batch_size_envs
