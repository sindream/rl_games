...
class ExperienceBuffer:
    ...
    def _init_from_env_info(self, env_info):
        ...
        # wj added
        self.tensor_dict['privileged_obses'] = self._create_tensor_from_space(env_info['priv_observation_space'], obs_base_shape)
        self.tensor_dict['estimate_states'] = self._create_tensor_from_space(env_info['estimate_state_space'], state_base_shape)
        self.tensor_dict['est_states'] = self._create_tensor_from_space(env_info['estimate_state_space'], state_base_shape)
        self.tensor_dict['p_boots'] = self._create_tensor_from_space(gym.spaces.Box(low=0, high=1,shape=(), dtype=np.uint8), obs_base_shape)
        ###
