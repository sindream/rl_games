...
class A2CBase(BaseAlgorithm):
    ...
    def get_action_values(self, obs):
        ...
        # wj added
        # privileged_obs_processed = self._preproc_obs(obs['privileged_obs'])
        # estimate_state_processed = self._preproc_obs(obs['estimate_state'])
        reward = obs['reward']
        ###
        ...
        input_dict = {
            ...
            # wj added
            # 'privileged_obs' : privileged_obs_processed,
            # 'estimate_state' : estimate_state_processed,
            'reward' : reward,
            ###
        }
    ...
    def obs_to_tensors(self, obs):
        ...
        if not obs_is_dict or 'obs' not in obs:    
            ...
            # wj added
            # Todo: add config for privileged_obs
            upd_obs = {'obs' : upd_obs['observations'],
                    #    'privileged_obs' : upd_obs['privileged_obs'],
                    #    'estimate_state' : upd_obs['estimate_state'],
                       'reward' : upd_obs['rewards'],
                       }
            ###
        return upd_obs
...
class ContinuousA2CPriv(ContinuousA2CBase):
    ...
    def init_tensors(self):
        A2CBase.init_tensors(self)
        self.update_list = ['actions', 'neglogpacs', 'values', 'mus', 'sigmas', 'est_states', 'p_boots']
        self.tensor_list = self.update_list + ['obses', 'states', 'dones', 'privileged_obses', 'estimate_states', 'rewards']
    ...
    def train_epoch(self):
        ...
        with torch.no_grad():
            if self.is_rnn:
                # batch_dict = self.play_steps_rnn()
                batch_dict = self.play_steps_rnn_prililiged()
            else:
                batch_dict = self.play_steps()
        ...
        for mini_ep in range(0, self.mini_epochs_num):
            ep_kls = []
            for i in range(len(self.dataset)):
                # print("train_epoch", self.dataset[i].keys())
        ...
        return batch_dict['step_time'], play_time, update_time, total_time, a_losses, c_losses, b_losses, entropies, kls, last_lr, lr_mul, e_losses
    ...
    def prepare_dataset(self, batch_dict):
        ...
        rnn_states = batch_dict.get('rnn_states', None)
        rnn_masks = batch_dict.get('rnn_masks', None)
        est_states = batch_dict['est_states']
        p_boots = batch_dict['p_boots']

        # wj added
        # print("batch_dict", batch_dict.keys())
        privileged_obses = batch_dict['privileged_obses']
        estimate_states = batch_dict['estimate_states']
        rewards = batch_dict['rewards']

        # ### active perception  # fail
        # # e_loss as reward
        # with torch.no_grad():
        #     # print("est_states", est_states.shape)
        #     # print("estimate_states", estimate_states.shape)
        #     # print("rewards", rewards.shape)
        #     e_loss = torch.mean((est_states.detach() - estimate_states.detach()) ** 2, dim=-1, keepdim=True)
        #     e_loss_reward = -torch.tanh(e_loss)
        #     rewards  = rewards + e_loss_reward * 30
        #     # print("e_loss_reward", e_loss_reward.shape)
        #     # print("rewards", rewards.shape)
        # ###
        ...
        dataset_dict['sigma'] = sigmas
        # print("obses", obses)
        dataset_dict['privileged_obs'] = privileged_obses        # mw added
        dataset_dict['estimate_state'] = estimate_states        # mw added
        dataset_dict['est_state'] = est_states        # mw added
        dataset_dict['reward'] = rewards        # mw added
    ...
    def train(self):
        ...
        while True:
            epoch_num = self.update_epoch()
            step_time, play_time, update_time, sum_time, a_losses, c_losses, b_losses, entropies, kls, last_lr, lr_mul, e_losses = self.train_epoch()
            total_time += sum_time
            frame = self.frame // self.num_agents
            ...
            if self.global_rank == 0:
                ...
                self.write_stats(total_time, epoch_num, step_time, play_time, update_time,
                                a_losses, c_losses, entropies, kls, last_lr, lr_mul, frame,
                                scaled_time, scaled_play_time, curr_frames, e_losses)
    ...
    def write_stats(self, total_time, epoch_num, step_time, play_time, update_time, a_losses, c_losses, entropies, kls, last_lr, lr_mul, frame, scaled_time, scaled_play_time, curr_frames, e_losses):
        # do we need scaled time?
        self.diagnostics.send_info(self.writer)
        self.writer.add_scalar('performance/step_inference_rl_update_fps', curr_frames / scaled_time, frame)
        self.writer.add_scalar('performance/step_inference_fps', curr_frames / scaled_play_time, frame)
    ...
    def play_steps_rnn_prililiged(self):
        update_list = self.update_list
        mb_rnn_states = self.mb_rnn_states
        step_time = 0.0
        ...
        for n in range(self.horizon_length):
            ...
            if self.use_action_masks:
                ...
                res_dict = self.get_action_values(self.obs) # t

            self.rnn_states = res_dict['rnn_states']
            self.experience_buffer.update_data('obses', n, self.obs['obs'])
            # wj added
            self.experience_buffer.update_data('privileged_obses', n, self.obs['privileged_obs'])
            self.experience_buffer.update_data('estimate_states', n, self.obs['estimate_state'])

            estimate_state = self.obs['estimate_state']
            ###
            self.experience_buffer.update_data('dones', n, self.dones.byte())

            # print("play step: ", self.obs.keys())
            ...
            step_time_start = time.time()
            self.obs, rewards, self.dones, infos = self.env_step(res_dict['actions']) # t

            # active perception
            next_res_dict = self.get_action_values(self.obs) # t+1
            e_loss = F.mse_loss(next_res_dict['est_states'],
                                self.obs['estimate_state'] + 10).detach()
            # e_loss_reward = -torch.tanh(e_loss)
            e_loss_reward = torch.clamp(-e_loss, min=-1.0)
            # e_loss_reward = torch.clamp(e_loss_reward, min=-1.0)
            # e_loss_reward = torch.clamp(e_loss_reward, min=-1.0, max=-0.2) + 0.2
            # print("e_loss_reward", e_loss)
            # rewards = rewards + e_loss_reward
            # print("rewards", rewards.shape)
            # print("done", self.dones.shape)
            # rewards = torch.where(self.dones.unsqueeze(1),
            #                        rewards, rewards + e_loss_reward/50)
            rewards = torch.where(self.dones.unsqueeze(1),
                                   rewards, rewards + e_loss_reward * 0.1)
    ...
    def get_values(self, obs):
        with torch.no_grad():
            if self.has_central_value:
                ...
                # wj added
                privileged_obs_processed = self._preproc_obs(obs['privileged_obs'])
                estimate_state = self._preproc_obs(obs['estimate_state'])
                reward = obs['reward']
                ###
                input_dict = {
                    'is_train': False,
