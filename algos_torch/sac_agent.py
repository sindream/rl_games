    ...
    def play_steps(self, random_exploration = False):
        ...
        for s in range(self.num_steps_per_episode):
            ...
            if isinstance(next_obs, dict):    
                next_obs_processed = next_obs['obs']['observations']
                next_obs = next_obs['obs']['observations']
...
class SACAgentPrivil(SACAgent):

    def __init__(self, base_name, params):
        super(SACAgentPrivil, self).__init__(base_name, params)
    ...
    def update_actor_and_alpha(self, obs, step):
        ...
        dist = self.model.actor(obs) # get estimated state here
        action = dist.rsample()
        log_prob = dist.log_prob(action).sum(-1, keepdim=True)
        entropy = -log_prob.mean() #dist.entropy().sum(-1, keepdim=True).mean()
        actor_Q1, actor_Q2 = self.model.critic(obs, action)
