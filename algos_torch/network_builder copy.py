    ...
    class Network(NetworkBuilder.BaseNetwork):
        ...
        def forward(self, obs_dict):
            ...
            if self.separate:
                ...
                """
                privileged_obses_c_out = obs_dict['privileged_obses_obs']
...
class A2CPrivilegedBuilder(NetworkBuilder):
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            self.a_rnn = self._build_rnn(self.rnn_name, 64+self.num_obs, self.rnn_units, self.rnn_layers)
            # self.c_rnn = self._build_rnn(self.rnn_name, 64+10, self.rnn_units, self.rnn_layers)
            ...
            mlp_args_for_privileged_obses_critic ={
                'input_size' : in_mpl_critic_shape + actions_num + self.num_obs,
                'units' : [512,256,128],
                # 'units' : [128,128],
                'activation' : self.activation,
                'norm_func_name' : self.normalization,
            ...
            mlp_encoder_args = {
                ...
                # 'units' : [128, 22], 
                'units' : [512, 256, 128 - self.num_obs], 
                'activation' : self.activation, 
                'norm_func_name' : self.normalization,
                'dense_func' : torch.nn.Linear,
                'd2rl' : self.is_d2rl,
            ...
            cnn_args = {
                'ctype': 'conv2d',  # 사용할 convolution 유형 (2D conv)
                'input_shape': (128, 15, 15),  # 채널 수 128, 공간적 크기 15x15
                'convs': [
                    {'filters': 64, 'kernel_size': 3, 'strides': 2, 'padding': 1},  # (128, 15, 15) -> (64, 8, 8)
                    {'filters': 32, 'kernel_size': 3, 'strides': 2, 'padding': 1},  # (64, 8, 8) -> (32, 4, 4)
                    {'filters': 16, 'kernel_size': 3, 'strides': 2, 'padding': 1},  # (32, 4, 4) -> (32, 2, 2)
                ],
                'activation': 'elu',  # 사용할 활성화 함수
                'norm_func_name': 'batch_norm',  # BatchNorm 사용
            }
        ...
        def forward(self, obs_dict):
            ...
            if self.separate:
                ...
                a_img = a_img.resize_(a_img.size(0), 128, 15, 15)
                a_img = self.actor_cnn(a_img)
                a_img = a_img.flatten(1)
                a_out = torch.cat([a_obs, a_img], dim=1)
                a_out2 = a_obs

                # # PRI CRITIC
                # c_out = privileged_obs
                # c_img = self.critic_cnn(c_img)
                # ORG CRITIC
                # c_img = c_img.resize(c_img.size(0), 128, 15, 15)
                # c_img = self.critic_cnn(c_img)
                # c_img = c_img.flatten(1)
                c_out = torch.cat([c_obs, c_obs2], dim=1)
                # c_out2 = c_obs
                # c_out2 = privileged_obs
                ###
                ...
                # replacement_values = (estimate_state / 10 - 1).detach()
                ...
                a_out = torch.cat([a_out, a_out2], dim=1)
                a_out = self.actor_mlp(a_out)

                # ORG CRITIC
                # print("c_out shape:", c_out.shape)
                # print("c_out2 shape:", c_out2.shape)
                # c_out = self.critic_encoder(c_out) # 22
                # print("c_out shape after encoder:", c_out.shape)


                # c_out = torch.cat([c_out, c_out2], dim=1) # 
                # c_out = self.critic_mlp(c_out)
                ...
                # c_out = torch.cat([c_out, a_out], dim=1)
                # print("c_out shape after cat:", c_out.shape)
                # c_out = self.critic_mlp(c_out)
                ...
                c_out = torch.cat([c_out, mu.detach()], dim=1) # 
                # c_out = torch.cat([c_out, c_out2, mu], dim=1) # 
                c_out = self.critic_mlp(c_out)

                # PRI CRITIC
                # c_out = torch.cat([c_out, mu], dim=1)
                # c_out = self.critic_mlp(c_out)
                ###
