...
class BaseModelNetwork(nn.Module):
    def __init__(self, obs_shape, normalize_value, normalize_input, value_size):
        ...
        if normalize_input:
            ...
            # wj addid
            if isinstance(obs_shape, dict):
                self.running_mean_std_privil = RunningMeanStdObs(20)
            else:
                self.running_mean_std_privil = RunningMeanStd(20)

    def norm_obs(self, observation):
        with torch.no_grad():
            return self.running_mean_std(observation) if self.normalize_input else observation

    def norm_priv_obs(self, observation):
        with torch.no_grad():
            return self.running_mean_std_privil(observation) if self.normalize_input else observation        
...
# wj added this class to support privileged
class ModelA2CContinuousLogStdPrivileged(BaseModel):
    def __init__(self, network):
        BaseModel.__init__(self, 'a2c')
        self.network_builder = network
    ...
    class Network(BaseModelNetwork):
        ...
        def forward(self, input_dict):
            ...
            # print(input_dict.keys()) # dict_keys(['is_train', 'prev_actions', 'obs', 'rnn_states', 'privileged_obs', 'estimate_state'])
            input_dict['privileged_obs'] = self.norm_priv_obs(input_dict['privileged_obs'])
            ###
            mu, logstd, value, states, est_states, p_boots = self.a2c_network(input_dict)
            sigma = torch.exp(logstd)
            distr = torch.distributions.Normal(mu, sigma, validate_args=False)
            if is_train:
                entropy = distr.entropy().sum(dim=-1)
